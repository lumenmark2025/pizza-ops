      ...
      <div className="grid gap-4 lg:grid-cols-4">
        {BOARD_STATUSES.map((status) => (
          <Card
            key={status}
            className={cn(
              'flex min-h-[24rem] max-h-[calc(100svh-11rem)] flex-col overflow-hidden rounded-lg border p-0 shadow-none',
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-4 pr-3 sm:px-5 sm:pb-5 sm:pr-4">
              {grouped[status].map((order) => (
                <div key={order.id} className="mb-3 overflow-hidden rounded-lg border border-slate-300 bg-white text-slate-950 shadow-none last:mb-0">
                  <div
                    className={cn(
                      'h-1.5',
                      status === 'taken' && 'bg-sky-500',
